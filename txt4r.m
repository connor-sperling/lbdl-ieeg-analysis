function txt4r(EEG, TvD, win_ms, lock_pth, subjs_dir)
    ...
    if ~exist(txt_pth, 'dir')
        if isunix
            unix(sprintf('sudo mkdir %s', txt_pth));
        else
            mkdir(txt_pth);
        end
    end
...
%             end
        ...
        if ~exist(fname, 'file') 
            % Header
            fid = fopen(fname, 'wt');
            sublist = sprintf('%s\t%s\tChannel\tRegion\tSegment\tAvg_%s', pred{1}{1}, pred{2}{1}, study);
            for i = 3:length(pred)
        ...
        for x = 1:xdiv
            for k = 1:size(y,1) 
                sublist = sprintf('%s\t%s\t%s\t%s\t%d\t%f', pred{1}{k+1}, pred{2}{k+1}, lab, region, x, y(k,x));
                for l = 3:length(pred) 
